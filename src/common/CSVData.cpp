...
void item_callback(void* s,size_t i,void* data) {
	...
	#ifdef _WIN32
		int size_needed = MultiByteToWideChar(CP_UTF8,0,static_cast<const char*>(s),static_cast<int>(i),NULL,0);
		std::wstring wcontent = L"";
		if(size_needed) {
			wcontent = std::wstring(size_needed,0);
			MultiByteToWideChar(CP_UTF8,0,static_cast<const char*>(s),static_cast<int>(i),&wcontent[0],size_needed);
		}
	#else
